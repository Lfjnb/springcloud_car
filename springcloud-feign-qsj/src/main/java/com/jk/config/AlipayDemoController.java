...
import com.jk.controller.DealCompleteController;
...
public class AlipayDemoController {
    @Autowired
    private  DealCompleteController dealCompleteController;
    ...
    @ResponseBody
    public String goAlipay(HttpServletRequest request, HttpServletRequest response,Integer  carid) throws Exception {

        Date date = new Date();
        Integer  time = (int)date.getTime();
        CarBean carBean = dealCompleteController.queryById(carid);

        carBean.setId(time);
        mongoTemplate.save(carBean);
        ...
        //付款金额，必填
        Double total_amount = carBean.getPrice();
        //订单名称，必填
        String subject = carBean.getCarName();
        //商品描述，可空
        String body = "";
